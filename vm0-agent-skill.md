...
1. **List schedules**: Run `npx -y @vm0/cli schedule ls` to show scheduled tasks
2. **List agents**: Run `npx -y @vm0/cli agent ls` to show deployed agents
...
**To get an agent's configuration from cloud, use `npx -y @vm0/cli agent clone`:**
...
- `npx -y @vm0/cli schedule ls` - List scheduled tasks
- `npx -y @vm0/cli agent ls` - List deployed agents
...
- Check if tokens are already stored remotely: `npx -y @vm0/cli secret list`
- If missing, guide user to obtain and paste new tokens
- **Store remotely via `npx -y @vm0/cli secret set`** (persists across runs and schedules)
...
**Important**: `npx -y @vm0/cli compose` is idempotent. If the configuration hasn't actually changed, the version hash stays the same - this is normal, not an error. Do not try flags like `--force` (they don't exist). If compose succeeds without error, the deployment is complete.

Verify with `npx -y @vm0/cli agent ls` to see the agent and its version.

### 7. Test Run

**Secrets and variables are stored remotely.** Check what's available with `npx -y @vm0/cli secret list` and `npx -y @vm0/cli variable list`.

**Run command** (do not guess additional flags):

```bash
npx -y @vm0/cli cook "your test prompt"
```

The `cook` command only supports these options: `-y` (skip confirmation). Do not add flags like `--artifact-name` (use `npx -y @vm0/cli run` for that).
...
- **New secrets required**: If new skills added that need secrets, store them via `npx -y @vm0/cli secret set` — they will be automatically available to scheduled runs
- **Timing change only**: Use `npx -y @vm0/cli schedule update` to modify frequency/time
- **No change needed**: If only AGENTS.md content changed with same skills, existing schedule continues to work after `npx -y @vm0/cli compose`
...
- [ ] For each token needed:
  ...
  - **Store remotely via `npx -y @vm0/cli secret set`** (persists across runs and schedules):
    ```bash
    npx -y @vm0/cli secret set SLACK_BOT_TOKEN --body "xoxb-xxx"
    ```
  - For non-sensitive config, use `npx -y @vm0/cli variable set`:
    ```bash
    npx -y @vm0/cli variable set ENV_NAME production
    ```

## Test run

- [ ] Explain in advance that the run may take a relatively long time, 1-20 minutes
- [ ] Use the vm0-cli skill capabilities to run the agent with cook: `npx -y @vm0/cli cook "your test prompt"`
- [ ] If the workflow writes files to the workspace, explain to the user where and how to view the artifact
- [ ] Explain to the user what command cook executed, and introduce the CLI capabilities of vm0 (using `npx -y @vm0/cli`)
...
- [ ] The run_id is returned after each `npx -y @vm0/cli run` command completes
...
>
> - **credentials**: Reserved for model provider authentication only (e.g., `CLAUDE_CODE_OAUTH_TOKEN`). These are stored persistently on the VM0 platform and managed via `npx -y @vm0/cli model-provider setup/set-default/delete`.
> - **secrets**: Use for ALL sensitive skill API keys and tokens. Store remotely via `npx -y @vm0/cli secret set` — they persist across runs and schedules.
> - **vars**: Use for non-sensitive configuration (feature flags, environment names). Store remotely via `npx -y @vm0/cli variable set`.
...
npx -y @vm0/cli secret set SLACK_BOT_TOKEN

# Store a secret (non-interactive)
npx -y @vm0/cli secret set NOTION_API_KEY --body "secret_xxx"

# Store a variable
npx -y @vm0/cli variable set ENV_NAME production

# List what's stored
npx -y @vm0/cli secret list
npx -y @vm0/cli variable list
```
...
```bash
npx -y @vm0/cli run my-agent "prompt" --secrets API_KEY=sk-xxx --vars ENV_NAME=production
```
...
3. **Ensure the value is stored remotely:**
   ```bash
   npx -y @vm0/cli secret set API_KEY
   ```
